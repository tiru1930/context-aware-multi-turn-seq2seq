...
class ModelCreator(object):
    ...
    def build_graph(self, hparams, scope=None):
        ...
        with tf.variable_scope(scope or "dynamic_seq2seq", dtype=dtype):
            ...
            #context
            context_outputs, context_state = self._build_context(hparams)
            # print(context_outputs)
            # print(context_state)

            concat_outputs = tf.concat([encoder_outputs,context_outputs],0)
            # concat_states = tf.reduce_sum([encoder_state,context_state],0)
            # print(context_outputs)

            # Decoder
            logits, sample_id, final_context_state = self._build_decoder(
                concat_outputs, encoder_state, hparams)
            # logits, sample_id, final_context_state = self._build_decoder(
            #     concat_outputs, concat_states, hparams)
    ...
    def _build_context(self,hparams):
        """building context vector"""
        context = self.batch_input.context
        if self.time_major:
        ...
        with tf.variable_scope("context") as scope:
            ...
            context_emb_inp = tf.nn.embedding_lookup(self.embedding, context)
            ...
            context_outputs, context_state = tf.nn.dynamic_rnn(
                ...
                context_emb_inp,
                dtype=dtype,
                sequence_length=self.batch_input.context_sequence_length,
