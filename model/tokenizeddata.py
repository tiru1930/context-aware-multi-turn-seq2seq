...
MAX_LEN = 5000  # Assume no line in the training data is having more than this number of characters
VOCAB_FILE = "vocab_babi_processed.txt"
DATA_FILE  = "babi_processed.txt"
...
class TokenizedData:
    ...
    def get_training_batch(self, num_threads=4):
        ...
        train_set = train_set.map(lambda src, tgt,cant:
                                  (src, tf.concat(([self.hparams.bos_id], tgt), 0),
                                   tf.concat((tgt, [self.hparams.eos_id]), 0),cant),
                                  num_parallel_calls=num_threads).prefetch(buffer_size)

        # Add in sequence lengths.
        train_set = train_set.map(lambda src, tgt_in, tgt_out,cant:
                                  (src, tgt_in, tgt_out, cant,tf.size(src), tf.size(tgt_in),tf.size(cant),tf.size(cant)+tf.size(src)),
                                  num_parallel_calls=num_threads).prefetch(buffer_size)
        ...
        def batching_func(x):
            return x.padded_batch(
                ...
                padded_shapes=(tf.TensorShape([None]),  # src
                               ...
                               tf.TensorShape([None]),  # context
                               tf.TensorShape([]),      # src_len
                               tf.TensorShape([]),      # tgt_len
                               tf.TensorShape([]),      
                               tf.TensorShape([])),     # context_seq_len
                ...
                padding_values=(self.hparams.eos_id,  # src
                                ...
                                self.hparams.eos_id,  # context
                                0,       # src_len -- unused
                                0,       # tgt_len -- unused
                                0,       # context_len -- unused
                                0))      
        ...
        if self.hparams.num_buckets > 1:
            ...
            def key_func(unused_1, unused_2, unused_3,unused_4,src_len, tgt_len,\
                                context_seq_len,dec_sequence_length):
        ...
        (src_ids, tgt_input_ids, tgt_output_ids,context_ids,src_seq_len, tgt_seq_len,\
                                context_seq_len,dec_sequence_length) = (batched_iter.get_next())
        ...
        return BatchedInput(initializer=batched_iter.initializer,
                            ...
                            context=context_ids,
                            context_sequence_length=context_seq_len,
                            decoder_sequence_length=dec_sequence_length,
                            source_sequence_length=src_seq_len,
                            target_sequence_length=tgt_seq_len)

    def get_inference_batch(self, src_dataset):
        text_dataset = src_dataset.map(lambda src,cont: (tf.string_split([src]).values,tf.string_split([cont]).values))
        # cont_dataset = cont_dataset.map(lambda cont: tf.string_split([cont]).values)

        if self.hparams.src_max_len_infer:
            text_dataset = text_dataset.map(lambda src,cont: (src[:self.hparams.src_max_len_infer],cont))
        # Convert the word strings to ids
        id_dataset = text_dataset.map(lambda src,cont: (tf.cast(self.vocab_table.lookup(src),
                                                          tf.int32),tf.cast(self.vocab_table.lookup(cont),
                                                          tf.int32)))
        # id_cont_dataset = cont_dataset.map(lambda cont: tf.cast(self.vocab_table.lookup(cont),
        #                                                   tf.int32))
        if self.hparams.source_reverse:
            id_dataset = id_dataset.map(lambda src,cont: (tf.reverse(src, axis=[0]),cont))
        # Add in the word counts.
        id_dataset = id_dataset.map(lambda src,cont: (src,cont,tf.size(src),tf.size(cont)))
        # id_cont_dataset = id_cont_dataset.map(lambda cont: (cont, tf.size(cont)))
        ...
        (src_ids,cont_ids,src_seq_len,cont_seq_len) = infer_iter.get_next()
        ...
        return BatchedInput(initializer=infer_iter.initializer,
                            ...
                            target_output=None,
                            context=cont_ids,
                            context_sequence_length=cont_seq_len,
    ...
    def load_corpus(self,corpus_dir):
        ...
        tgt_dataset = tgt_dataset.map(lambda line:
                                      tf.substr(line, 2, MAX_LEN)).prefetch(4096)
        context_dataset = dataset.filter(lambda line:
                                     tf.logical_and(tf.size(line) > 0,
                                                    tf.equal(tf.substr(line, 0, 5), tf.constant('cont:'))))
        context_dataset = tgt_dataset.map(lambda line:
                                      tf.substr(line, 2, MAX_LEN)).prefetch(4096)

        src_tgt_dataset = tf.data.Dataset.zip((src_dataset, tgt_dataset, context_dataset))
